...
import { CourseContentGateway } from './course-content.gateway';
import {
  CourseModule,
...
/**
 * TEMPORARY MOCK - see CourseContentGateway for the swap-to-real-backend plan.
 *
...
export class CourseContentMockService extends CourseContentGateway {
  ...
  private media: MediaAsset[] = [
    {
      ...
      url: 'https://picsum.photos/seed/quali-modulo-1/640/360',
      mimeType: 'image/jpeg',
    ...
    },
    {
      ...
      courseId: 1,
      type: 'DOCUMENT',
      name: 'apostila-introducao.pdf',
      url: 'https://example.org/docs/apostila-introducao.pdf',
    ...
    },
    {
      ...
      courseId: 2,
      type: 'IMAGE',
      name: 'capa-fundamentos.jpg',
      url: 'https://picsum.photos/seed/quali-modulo-3/640/360',
      mimeType: 'image/jpeg',
  ...
  private simulate<T>(value: T): Observable<T> {
    return of(value).pipe(delay(CourseContentMockService.LATENCY_MS));
  }

  private notFound<T>(what: string, id: number): Observable<T> {
    return throwError(() => new Error(`${what} ${id} não encontrado(a).`)).pipe(delay(CourseContentMockService.LATENCY_MS));
  }
