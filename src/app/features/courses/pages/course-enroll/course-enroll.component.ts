...
import { CourseEnrollmentMockService } from '../../services/course-enrollment-mock.service';
...
/**
 * "Matricular aluno" screen.
 *
 * The student list here is REAL: it queries StudentService.list(), the
 * exact same endpoint used by the Talentos screen. Only the enrollment
 * action itself (the "Matricular" button) is mocked - see
 * CourseEnrollmentMockService for why.
 */
@Component({
...
export class CourseEnrollComponent implements OnInit {
  ...
  fetch(page = 0): void {
    ...
    forkJoin({
      ...
      // TODO: swap for this.courseService.getEnrolledStudents(this.courseId) once
      // quali-courses-api returns real enrollment data - see CourseEnrolledStudentsMockService.
      enrolled: this.enrolledStudentsMock.getEnrolledStudents(this.courseId)
    })
  ...
  isEnrolled(student: Student): boolean {
    return this.alreadyEnrolledIds.has(student.id) || this.enrollmentMock.isMockEnrolled(this.courseId, student.id);
  }
  ...
  enroll(student: Student): void {
    ...
    this.enrollmentMock.enroll(this.courseId, student.id)
      .pipe(finalize(() => (this.enrollingId = null)))
      .subscribe(() => {
        this.notification.success(
          this.translate.instant('courses.enroll.notifications.enrolled', { name: student.fullName ?? '' })
        );
      });
  }
}
