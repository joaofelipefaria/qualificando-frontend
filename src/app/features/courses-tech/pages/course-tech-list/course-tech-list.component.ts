...
@Component({
  ...
  templateUrl: './course-tech-list.component.html',
  styleUrl: './course-tech-list.component.scss'
})
export class CourseTechListComponent implements OnInit {
  ...
  fetch(): void {
    this.loading = true;
    this.courseTechService.list(0, FETCH_ALL_SIZE, '')
  ...
  confirmDelete(course: CourseTech, event: Event): void {
    event.stopPropagation();
    const ref = this.dialog.open(ConfirmDialogComponent, {
      data: {
        title: this.translate.instant('coursesTech.form.deleteTitle'),
        message: this.translate.instant('coursesTech.form.deleteMessage', { name: course.name })
      }
    });

    ref.afterClosed().subscribe(confirmed => {
      if (!confirmed) return;
      this.courseTechService.delete(course.id).subscribe({
        next: () => {
          this.notification.success(this.translate.instant('coursesTech.notifications.deleted'));
          this.fetch();
        }
      });
    });
