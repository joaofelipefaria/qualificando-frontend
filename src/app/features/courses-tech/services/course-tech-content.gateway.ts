...
/**
 * Contract for everything related to tech course modules, their textual/media
 ...
 *
 * TODAY this is backed by `CourseTechContentMockService`, an in-memory
 ...
 * endpoints for modules/content/media, create a `CourseTechContentHttpService`
 * that implements this same class using HttpClient, and flip the provider
 * in `app.config.ts`:
 *
 *   { provide: CourseTechContentGateway, useClass: CourseTechContentHttpService }
 *
 * No component or template needs to change - they all depend on
 * `CourseTechContentGateway`, never on the mock directly.
 */
export abstract class CourseTechContentGateway {
  // ----- Modules -----
